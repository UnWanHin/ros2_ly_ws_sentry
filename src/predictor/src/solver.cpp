...
namespace ly_auto_aim::inline solver {
    // [ROS 2] 定義全局變量
    rclcpp::Node::SharedPtr global_predictor_solver_node = nullptr;
}

// [ROS 2] 本地 roslog 適配
namespace roslog_fmt {
    static rclcpp::Logger get_logger() {
        if (ly_auto_aim::solver::global_predictor_solver_node) {
            return ly_auto_aim::solver::global_predictor_solver_node->get_logger();
        }
        return rclcpp::get_logger("solver");
    }
...
// [ROS 2] 構造函數實現
Solver::Solver() {
    if (!global_predictor_solver_node) {
        std::cerr << "Solver Error: global_predictor_solver_node is NULL" << std::endl;
        return;
    }
    auto node = global_predictor_solver_node;
